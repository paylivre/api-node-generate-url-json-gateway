...
function getUrlGateway(DataURL, signature) {
  ...
  const pix_key_type = DataURL?.pix_key_type
    ...
    : "";
  const pix_key = DataURL?.pix_key ? `&pix_key=${DataURL.pix_key}` : "";
  const logoUrl = DataURL.logo_url ? `&logo_url=${DataURL.logo_url}` : "";
  const redirect_url = DataURL.redirect_url
    ? `&redirect_url=${DataURL.redirect_url}`
    : "";

  const UrlGateway = `${base_url}?${merchant_transaction_id}&${merchant_id}&${operation}${email}${document_number}&${amount}&${currency}&${mock_type}&${account_id}&${calback_url}&${auto_approve}${redirect_url}${Signature}${logoUrl}${pix_key_type}${pix_key}`;

  return {
    url: UrlGateway,
    merchant_transaction_id: merchant_transaction_id_random,
  };
}
...
//Mock response transaction
app.post("/api/v2/gateway", async (req, res) => {
  const mock_response = { message: "401" };
  res.status(401).send(mock_response);
});
